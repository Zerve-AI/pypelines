...
import os
import pyperclip
from .templates.pyod_dataprep import AnomalyDetectionTemplate
from .schemas import HyperParamsAD, NumericalParamAD, CategoricalParamAD
from .models.anomaly_detection import models_ad, models_comparison_ad, models_ad_default, models_comparison_ad_default
import pandas as pd 
...
class AnomalyDetectionPipeline:
        ...
        for k, v in params.items():
            for p in v:
                ...
                if k=='categorical':
                    if not p['selected']:
                        continue
                    hyperparams.append(CategoricalParamAD(**{'prefix': model_prefix, 'name': p['name'], 'values': p['selected']}))
                elif k=='numerical':
                    hyperparams.append(NumericalParamAD(**{'prefix': model_prefix, **p}))
        return HyperParamsAD(**{'params': hyperparams})
    ...
    def parse_config(self):
        ...
        self.default_imports = ad_imports
